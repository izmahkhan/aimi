    ...
    if ($_REQUEST['mode'] == 'add') {
        ...
        if (count($_FILES['img_files']['name']) > 0) {
            ...
            foreach ($_FILES['img_files']['name'] as $key => $extra) {
                ...
                $s1_max_orderid = "select max(gal_order) as orderid from tbl_gallery where gal_type='acls' ";
...
if (isset($_POST)) {
    ...
    $query_rs_pages = "SELECT * FROM tbl_gallery WHERE gal_type='acls' ORDER BY gal_order ASC";
